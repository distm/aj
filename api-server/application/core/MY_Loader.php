...
class MY_Loader extends CI_Loader {
    ...
    function content($content_name, $vars = '', $return = FALSE)
    {
        if(!$content_name)
        {
            return FALSE;
        ...
        $CI = & get_instance();
        $content_name = $CI->router->fetch_class() . "/" . $content_name;

        if($return === TRUE)
        {
            return $CI->load->view($content_name, $vars, TRUE);
        }
        else
        {
            $CI->load->view($content_name, $vars);
        }
    }
